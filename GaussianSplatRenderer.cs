...
// 6. Add conditional compilation check for XR
#if !UNITY_PIPELINE_URP
using XRSystem = UnityEngine.XR.XRSettings;
#else
// URP has different XR handling
using XRSystem = UnityEngine.XR.XRSettings;
#endif
using System;
...
namespace GaussianSplatting.Runtime
{
    public class GaussianSplatRenderSystem
    {
        ...
        **/
        ...
        public void RegisterSplat(GaussianSplatRenderer r)
        {
            if (m_Splats.Count == 0)
            {
                // Only hook into built-in pipeline
                if (GraphicsSettings.currentRenderPipeline == null)
                    Camera.onPreCull += OnPreCullCamera;
                // For URP/HDRP, the render feature/volume override handles rendering
            }
    ...
    public class GaussianSplatRenderer : MonoBehaviour
    {
        ...
        internal void CalcViewData(CommandBuffer cmb, Camera cam, Matrix4x4 matrix)
        {
            ...
            // VR: Get proper screen dimensions
            int screenW = cam.pixelWidth, screenH = cam.pixelHeight;

#if UNITY_PIPELINE_URP
    // For VR, use XR texture dimensions if available
    if (XRSettings.enabled)
    {
        ...
        screenH = XRSettings.eyeTextureHeight;
            ...
            if (eyeW != 0) screenW = eyeW;
            if (eyeH != 0) screenH = eyeH;
#endif
        ...
        public void Update()
        {
            ...
            // VR: Update center eye camera and matrix
            if (m_CenterEyeOnly)
            {
                // Try to find the main camera if we don't have one
                if (m_centerEyeCamera == null)
                    m_centerEyeCamera = Camera.main;

                if (m_centerEyeCamera != null)
                {
                    // For VR, calculate center eye position between both eyes
#if UNITY_PIPELINE_URP
            if (XRSystem.IsXREnabled())
            {
                // Get center eye matrix for VR
                m_centerCamMatrix = GetCenterEyeMatrix(m_centerEyeCamera);
            }
            else
#endif
                    {
        ...
        #if UNITY_PIPELINE_URP
        private Matrix4x4 GetCenterEyeMatrix(Camera cam)
        {
            // For single-pass instanced rendering, we want the center between both eyes
            if (XRSettings.enabled && XRSettings.stereoRenderingMode == XRSettings.StereoRenderingMode.SinglePassInstanced)
            {
                // Calculate center point between both eye positions
                Matrix4x4 leftEyeMatrix = cam.GetStereoViewMatrix(Camera.StereoscopicEye.Left);
                Matrix4x4 rightEyeMatrix = cam.GetStereoViewMatrix(Camera.StereoscopicEye.Right);
        
                // Extract positions
                Vector3 leftPos = leftEyeMatrix.inverse.GetColumn(3);
                Vector3 rightPos = rightEyeMatrix.inverse.GetColumn(3);
                Vector3 centerPos = (leftPos + rightPos) * 0.5f;
        
                // Use the camera's rotation with center position
                Matrix4x4 centerMatrix = cam.worldToCameraMatrix;
                Vector3 forward = centerMatrix.GetColumn(2);
                Vector3 up = centerMatrix.GetColumn(1);
        
                // Reconstruct view matrix with center position
                return Matrix4x4.LookAt(centerPos, centerPos + forward, up);
            }
